...
func (dnsDel *DNSDelType) Init(verbose bool, jsonOutput bool, backend string, id string, secret string, args []string) error {
	...
	dnsDel.P, err = setProvider(backend, id, secret, verbose)
	if err != nil {
		return err
	}
	return nil
...
func (dnsDel *DNSDelType) PrintParams() {
	if dnsDel.Verbose {
		...
		fmt.Fprintf(os.Stderr, "Password / Secret '%.2s' \n", dnsDel.Secret)
