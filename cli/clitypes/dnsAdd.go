...
func (dnsAdd *DNSAddType) Init(verbose bool, jsonOutput bool, backend string, force bool, id string, secret string, args []string) error {
	...
	dnsAdd.P, err = setProvider(backend, id, secret, verbose)
	return err
}
...
func (dnsAdd *DNSAddType) PrintParams() {
	if dnsAdd.Verbose {
		...
		fmt.Fprintf(os.Stderr, "Password 	'%.2s' \n", dnsAdd.Secret)
